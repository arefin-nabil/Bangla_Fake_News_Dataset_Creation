...
CHANNELS = [
    ...
    # {
    #     "url":      "https://www.youtube.com/@some_satire_channel",
    #     "label":    "fake",
    #     "category": "satire",
    # },
...
CSV_FIELDS = [
    "id", "title", "text", "source", "publish_date",
    ...
    try:
        ...
        if resp.status_code == 403:
            ...
            if reason == "quotaExceeded":
                log.error("❌  Daily quota exceeded! Come back tomorrow or use a different API key.")
                raise SystemExit(1)
...
def get_playlist_video_ids(playlist_id: str, page_token: str = None) -> tuple[list[dict], str | None]:
    """
...
def scrape_channel(ch_info: dict, visited: set, id_counter: list,
                   max_videos: int | None) -> list[dict]:
    """
    Scrape ALL videos from one channel.
    ...
    log.info(f"  ✅ Channel: '{ch_title}'  ID: {ch_id}  label={label}")
...
def run(max_videos: int | None = None, fresh: bool = False):
    ...
    for ch_info in CHANNELS:
        records = scrape_channel(ch_info, visited, id_counter, max_videos)
